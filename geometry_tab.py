...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QSplitter,
                             ...
                             QStackedWidget, QComboBox, QGroupBox, QFormLayout,
...
class GeometryTab(QWidget):
    ...
    def build_scale_page(self):
        ...
        info_lbl = QLabel("<i>Scales and centers the geometry to fit within the blockMesh dimensions defined in the Mesh Generation tab.</i>")
    ...
    def add_geometry(self):
        file_paths, _ = QFileDialog.getOpenFileNames(
            self, "Select Geometry Files", "", "Geometry Files (*.stl *.obj);;All Files (*)"
        )
        for path in file_paths:
            ...
            try:
                ...
                QMessageBox.warning(self, "Error", f"Failed to load {base_name}:\n{str(e)}")
    ...
    def apply_scale_to_mesh(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", "Could not read background mesh dimensions.")
            return
        ...
        if mesh_dx <= 0 or mesh_dy <= 0 or mesh_dz <= 0:
            QMessageBox.warning(self, "Error", "Buffer is too large for the mesh domain.")
            return
        ...
        geom_mesh = geom_mesh.scale([scale_factor, scale_factor, scale_factor], inplace=False)
        ...
        self.update_geometry_properties(name)
        QMessageBox.information(self, "Success", f"Scaled by factor: {scale_factor:.4f}")
    ...
    def save_geometry(self):
        ...
        if mesh is not None and name is not None:
            ...
            ext = os.path.splitext(source_path)[1].lower() if source_path else os.path.splitext(name)[1].lower()
            if not ext:
                ext = ".stl"
