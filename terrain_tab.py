...
class TerrainTab(QWidget):
    ...
    def setup_ui(self):
        ...
        if rasterio is None:
            warning_lbl = QLabel("⚠️ <b>rasterio</b> is not installed. Please run <i>pip install rasterio</i> in your terminal.")
        ...
        if rasterio is None: self.btn_load_tif.setEnabled(False)
        ...
        self.spin_factor.setToolTip("Downsample the grid (e.g. 2 skips every other pixel) to reduce triangle count.")
        ...
        self.spin_extrude.setToolTip("Depth below the lowest terrain point to create the flat base.")
        ...
        if rasterio is None: self.btn_generate.setEnabled(False)
        ...
        if path and rasterio:
            ...
            try:
                with rasterio.open(path) as src:
                    self.lbl_res.setText(f"Resolution: {src.width} x {src.height} pixels")
                    self.lbl_crs.setText(f"CRS: {src.crs}")
                    self.lbl_bounds.setText(f"Bounds: {src.bounds.left:.2f}, {src.bounds.bottom:.2f} "
    ...
    def generate_stl(self):
        if rasterio is None:
            QMessageBox.warning(self, "Warning", "rasterio is not installed. Install it to process DEM files.")
            return
        ...
        try:
            ...
            # Transformations
            solid = solid.translate((self.spin_dx.value(), self.spin_dy.value(), self.spin_dz.value()), inplace=False)
            ...
            save_path, _ = QFileDialog.getSaveFileName(
                self, "Save Terrain Solid", os.path.join(default_dir, "terrain.stl"), "STL Files (*.stl)"
            )

            if save_path:
                solid.save(save_path)
                QMessageBox.information(self, "Success", f"Terrain generated and saved to:\n{save_path}\n\n"
